...
def define_list():
    ...
    리스트 생성 관련 샘플 코드
    """
    lst1 = list()   # list 타입 함수를 이용한 생성
    print(lst1, type(lst1))
    lst2 = []   # [] -> list 표기법 *추천하는 생성방법*
    lst3 = [1, 2, "Python"] # 객체 타입을 가리지 않는다
    print(lst2, lst3)
...
def list_oper():
    """
    리스트는 len, 인덱싱, 슬라이싱, in, not in, 내부 데이터 치환 가능
    리스트 연산 연습
    """
    lst = [1, 2, "Python"]
    ...
    print(lst[1:])  # 끝 경계가 생략되면 끝까지 출력한다
    print(lst[0:2])  # == [1, 2]
    print(lst[:2])   # 시작 경계가 생략되면 처음부터 출력한다

    cp = lst[:] # 원본 전체의 copy본이 된다 -> 슬라이스를 활용한 전체의 복제
    print(cp)
...
def list_methods():
    ...
    # 반전(reverse)
    print("REVERSED:", list(reversed(lst_copy)))    # revered : 원본 유지 데이터 순서 반전 -> 새 리스트
    ...
    print("lst_copy sorted:", sorted(lst_copy, reverse=True))   # 내림차순 정렬
    print("lst_copy:", lst_copy)    # 내부 데이터 변경되지 않음
    ...
    # sorted 함수: 원본 유지, 데이터 정렬한 새 리스트를 반환
    # sort method: 내부 데이터를 실제로 정렬함
