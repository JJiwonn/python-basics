def define_tuple():
    """
    튜플의 정의
    tuple:- list와 거의 동일하지만 변경불가 자료형이다(immutable)
    ...
    print(tp1, type(tp1))
    print(tp2, type(tp2))
    tp3 = (1,)  # 요소가 1개일 경우 요소뒤에 콤마(,)를 찍어줘서 튜플로 인식한다
    print(tp3, type(tp3))
...
def tuple_oper():
    ...
    slice = tp[:]   # 처음부터 -> 생략가능 / 끝까지 -> 생략가능 /  [:] -> 튜플 전체
    print(slice,type(slice))
    ...
    # 포함 여부: in, not in
    print(5 in tp)  # 5가 포함 되어 있나요?
    print(1 not in tp)  # 1이 포함 안되어 있나요?

def tuple_assign():
    """
    튜플의 할당
    """
    # 튜플을 이용, 좌우변에 여러 값을 동시에 할당
    x, y, z = 10, 20, 30    # 튜플
    print(x, y, z)
...
def tuple_method():
    ...
    """
    tp = (20, 30, 10, 12)
    print("count:", tp.count(20))   # 요소 갯수 세기
